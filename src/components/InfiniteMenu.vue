...
const emit = defineEmits(["item-change", "movement-change", "navigate"]);

// 水平线操作

const verticalLinePosition = ref(50); // 默认在50vw位置
...
const handleMouseMove = (event) => {
  ...
  const percentageX = (mouseX / viewportWidth) * 100;
  const percentageY = (mouseY / viewportHeight) * 100;
  verticalLinePosition.value = percentageX;
  horizontalLinePosition.value = percentageY;
};
...
const verticalLineStyle = computed(() => {
  if (isMobile.value) {
    ...
    return {
      left: `${verticalLinePosition.value}vw`,
      height: "100vh",
    };
  }
});
...
class InfiniteGridMenu {
  ...
  onControlUpdate(deltaTime) {
    ...
    if (isMovingNow !== this.movementActive) {
      this.movementActive = isMovingNow;
      this.onMovementChange(isMovingNow);
    }
  ...
  setRandomInitialRotation() {
    ...
    const randomIndex = Math.floor(
      Math.random() * this.instancePositions.length
    );

    // 获取随机顶点位置
    const randomVertexPos = this.instancePositions[randomIndex];

    // 目标方向（屏幕正前方）
    const targetDirection = vec3.fromValues(0, 0, -1);

    // 计算从随机顶点到目标方向的旋转四元数
    const normalizedVertex = vec3.normalize(vec3.create(), randomVertexPos);
    ...
    // 计算旋转角度
    const dot = vec3.dot(normalizedVertex, targetDirection);
    const angle = Math.acos(Math.max(-1, Math.min(1, dot)));

    // 设置初始旋转四元数
    if (vec3.length(rotationAxis) > 0.0001) {
      quat.setAxisAngle(this.control.orientation, rotationAxis, angle);
    }

    // 触发初始的active item更新
    const itemIndex = randomIndex % Math.max(1, this.items.length);
    this.onActiveItemChange(itemIndex);
  }
}
...
const handleActiveItem = (index) => {
  ...
  emit("item-change", activeItem.value);
};

// Handle movement change
const handleMovementChange = (moving) => {
  isMoving.value = moving;
  emit("movement-change", moving);
};
...
onMounted(() => {
  ...
  // 为fixed-title内的图片添加随机动画延迟
  const fixedTitleImages = document.querySelectorAll(".fixed-title img");
  fixedTitleImages.forEach((img, index) => {
    // 为每个图片设置随机的动画延迟
...
watch(
  ...
  (newItems) => {
    ...
    const canvas = canvasRef.value;
    if (canvas) {
      const items = newItems.length ? newItems : defaultItems;
      sketch = new InfiniteGridMenu(
        canvas,
        items,
        handleActiveItem,
      ...
      fixedTitleImages.forEach((img, index) => {
        ...
        img.style.animation = `flickerIn 1s ease-out forwards`;
        img.style.animationDelay = `${randomDelay}s`;
      });
    }
  },
