...
class InfiniteGridMenu {
  ...
  onControlUpdate(deltaTime) {
    ...
    const isMovingNow = this.control.isPointerDown || Math.abs(this.smoothRotationVelocity) > 0.01
  ...
  setRandomInitialRotation() {
    ...
    // 生成随机索引
    const randomIndex = Math.floor(Math.random() * this.instancePositions.length)
    ...
    // 计算旋转轴（两个向量的叉积）
    const rotationAxis = vec3.cross(vec3.create(), normalizedVertex, targetDirection)
